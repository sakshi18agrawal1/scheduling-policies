package os_a3_final;

public class priority_preemptive 
{
	int pid[];
	int burst[];
	int priority[];
	int waiting[];
	int arrival[];
	int extra[];
	
	
	public void find_waiting(int start)
	{
		int min=0;
		waiting= new int[burst.length+1];
		waiting[0]=0;
		int time=start,finish=0;
		int x=0,y=0,i=1;
		int completed_pid=0;
		while(completed_pid!=pid.length)
		{
			while(y<pid.length&&arrival[y]<=time)
			{
				y++;
			}
			//sort(x,y);
			
			min=find_min(y-1);
			if(min!=-1)
			{
				extra[min]--;
				System.out.print("P"+pid[min]+"("+time+"-"+(time+1)+") |\t");
				if(time%12==0)
					System.out.println();
				if(extra[min]==0)
				{
					completed_pid++;
					finish=time+1;
					waiting[min]=finish-burst[min]-arrival[min];
					//total_wait+= waiting[min];
				}
			}
			time++;
		}
		waiting[pid.length]= time;
	}
		
	public void print()
	{
		for(int i=0;i<pid.length;i++)
			System.out.println(pid[i]+" "+arrival[i]+" "+burst[i]+" "+priority[i]+" "+extra[i]);
	}
	public void copy()
	{
		for(int i=0;i<pid.length;i++)
			extra[i]=burst[i];
	}
	public int find_min(int a)
	{
		int min=100;
		int index=0,flag=0;
		for(int i=0;i<=a;i++)
		{
			if(priority[i]<min&&extra[i]>0)
			{
				min=priority[i];
				index=i;
				flag=1;
			}
		}
		if(flag==0)
			return -1;
		return index;
	}
	public int[] get(int p[],int b[],int prior[], int a[],int start)
	{
		pid=p;
		burst=b;
		priority= prior;
		arrival=a;
		extra=new int[pid.length];
		copy();
		find_waiting(start);
		return waiting;
	}
}
